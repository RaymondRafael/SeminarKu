...
<style>
    ...
    .dashboard-main {
        flex-grow: 1;
        padding: 30px; /* Jarak aman dari sidebar & atas */
    ...
    .checkout-grid {
        display: grid;
        grid-template-columns: 1.8fr 1fr; /* Kiri 65%, Kanan 35% */
        gap: 25px;
        align-items: start;
    }
    ...
    .total-label { font-weight: 600; color: var(--text-dark); }
    ...
    .qr-wrapper {
        ...
        align-items: center;       /* Ini yang bikin gambar ke tengah horizontal */
    ...
    .btn-confirm:hover { background-color: #354a96; transform: translateY(-1px); }
...
<div class="dashboard-container">
    <main class="dashboard-main">
        ...
        <div class="header-section">
            ...
            <p class="header-subtitle">Selesaikan pendaftaran untuk event: <strong>{{ $kegiatan->nama_kegiatan }}</strong></p>
        </div>
            ...
            <div class="checkout-grid">
                ...
                <div class="left-col">
                    <div class="card-white">
                        ...
                        {{-- Alert Error --}}
                        @if ($errors->any())
                            <div class="alert alert-danger" style="background:#fde8e8; color:#c81e1e; padding:10px; border-radius:8px; margin-bottom:15px; font-size:0.9rem;">
                        ...
                        <p style="margin-bottom: 15px; color: var(--text-grey); font-size: 0.9rem;">Pilih sesi yang ingin diikuti:</p>

                        @foreach ($sesiKegiatans as $sesi)
                            <label class="session-card" for="sesi-{{ $sesi->id_detail_kegiatan }}">
                                <input type="checkbox" 
                                       id="sesi-{{ $sesi->id_detail_kegiatan }}"
                                       name="id_detail_kegiatan[]" 
                                       value="{{ $sesi->id_detail_kegiatan }}" 
                                       class="custom-checkbox session-check"
                                       data-price="{{ $sesi->biaya_registrasi }}"
                                       checked> {{-- Default Checked agar user langsung lihat harga --}}
                                
                                <div class="session-info">
                                    <span class="session-name">Sesi {{ $sesi->sesi }}: {{ $sesi->nama_sesi }}</span>
                                    <span class="session-date">
                                        <i class="far fa-calendar-alt"></i> {{ \Carbon\Carbon::parse($sesi->tanggal)->format('d M Y') }} • 
                                        <i class="far fa-clock"></i> {{ \Carbon\Carbon::parse($sesi->waktu_mulai)->format('H:i') }}
                                    </span>
                                </div>
                ...
                <div class="right-col">
                    <div class="card-white">
                        <div class="timer-alert" id="countdown">
                            Sisa Waktu: 23:59:59
                        </div>
                        ...
                        <div class="qr-wrapper">
                            {{-- Gambar QRIS Statis --}}
                            <img src="{{ asset('images/qris-panitia.jpg') }}" 
                                 alt="QRIS Pembayaran" 
                                 style="width: 180px; height: 180px; object-fit: contain;">
                            
                            <p style="margin-top:10px; font-size:0.8rem; color:#666;">
                                Scan menggunakan GoPay/OVO/Dana/BCA.<br>
                                <strong>Pastikan nominal sesuai total!</strong>
                            </p>
                        </div>
                        ...
                        <div class="upload-area">
                            <input type="file" name="bukti_pembayaran" id="fileInput" accept="image/*, application/pdf" hidden required>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // 3. LOGIC TIMER MUNDUR (DIUBAH JADI 30 MENIT)
        // 30 menit x 60 detik = 1800 detik
        let duration = 30 * 60; 
        ...
        const timerInterval = setInterval(() => {
            ...
            timerEl.textContent = `Sisa Waktu: ${h < 10 ? '0'+h : h}:${m < 10 ? '0'+m : m}:${s < 10 ? '0'+s : s}`;
            
            if (duration > 0) {
                duration--;
            } else {
                // Opsional: Apa yang terjadi jika waktu habis?
                clearInterval(timerInterval);
                timerEl.textContent = "Waktu Habis!";
                timerEl.style.backgroundColor = "#ffebee";
                timerEl.style.color = "#c62828";
                
                // Bisa tambahkan disable tombol submit jika mau
